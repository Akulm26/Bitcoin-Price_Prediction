...
```{r}
df = read.csv("/Users/akul/Desktop/Bitcoin/bitstampUSD_1-min_data_2012-01-01_to_2021-03-31.csv")
attach(df)
head(df)
dim(df)
```
...
library(modelr)
data.frame(
  R2 = rsquare(model, data = test),
  RMSE = rmse(model, data = test),
  MAE = mae(model, data = test)
)
```
